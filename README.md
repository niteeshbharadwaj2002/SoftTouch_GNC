...
![Python](https://img.shields.io/badge/python-3.13-blue) ![License](https://img.shields.io/badge/license-PLACEHOLDER-lightgrey)
...
<!-- PLACEHOLDER: fill these in from your own `python main.py` run output ("--- Results ---" block) -->
**Landing accuracy:** `PLACEHOLDER m` &nbsp;|&nbsp; **Fuel used vs. planned:** `PLACEHOLDER kg (±PLACEHOLDER%)` &nbsp;|&nbsp; **Max tracking error under wind gust:** `PLACEHOLDER m`
...
<!-- PLACEHOLDER TABLE: run `python main.py`, copy the "--- Results ---" and "--- Estimation (Kalman) ---" console blocks into this table -->

| Metric | Value |
|---|---|
| Landing accuracy (true, from target) | `PLACEHOLDER m` |
| Planned fuel use | `PLACEHOLDER kg` |
| Actual fuel use | `PLACEHOLDER kg` |
| Max tracking error during flight | `PLACEHOLDER m` |
| Final Kalman x-estimation error (unobserved) | `PLACEHOLDER m` |
| Final Kalman y-estimation error (observed) | `PLACEHOLDER m` |
| Guidance solve time | `PLACEHOLDER s` |
...
- **No hardware-in-the-loop or real-time execution constraints.** Guidance solves once, offline, before the flight starts, not in a receding-horizon/replanning loop as an operational system would require.
